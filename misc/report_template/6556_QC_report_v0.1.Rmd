...
* Gene models `Homo_sapiens.GRCh38.109.gtf` were obtained from Ensembl and used for transcriptome quantification by `salmon`;
...
* Gene model annotations from **Gencode v 44** were used for gene fusion detection by `JAFFAL`.
...
This command was used as in the differential expression module of the original `wf-transcriptomes`;
...
* For alignment statistics, reads were mapped to the reference genome using `minimap2` outputting all reads (i.e. mapped and unmapped) and the alignments were filtered and counted using `samtools` for an estimate of number of "good quality alignments". The filtering was applied only to obtain read mapping statistics for summary table; *Primary alignments* were filtered based on MAPQ (alignment quality) and the cutoff was `-q 60`.

* Alignment quality filtering applied in the processing workflow was as per default settings for each tool, and the corresponding alignment numbers are not listed in the summary table. 
